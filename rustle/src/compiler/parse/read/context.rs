...
use crate::compiler::parse::utils::{
...
use swc_estree_ast::{BaseNode, Identifier, PatternLike};
...
pub fn read_context(mut parser: Parser) -> (PatternLike, u32, u32) {
    ...
    let other_code = full_char_code_at(&parser.template, i);
    ...
    while i < parser.template.len() {
        ...
        if is_bracket_open(code) {
            ...
            if !is_bracket_pair(bracket_stack[bracket_stack.len() - 1], code) {
                let close_bracket =
                    get_bracket_close(bracket_stack[bracket_stack.len() - 1]).unwrap() as u16;
                let error = Error::unexpected_token(&String::from_utf16(&[close_bracket]).unwrap());
                parser.error(&error.code, &error.message, None);
            }
            bracket_stack.pop();
