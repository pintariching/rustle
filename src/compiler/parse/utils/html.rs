...
lazy_static! {
    ...
    static ref DISSALLOWED_CONTENTS: HashMap<&'static str, Vec<&'static str>> = HashMap::from([
        ("li", vec!["li"]),
        ("dt", vec!["dt", "dd"]),
        ("dd", vec!["dt", "dt"]),
...
pub fn closing_tag_ommited(current: &str, next: Option<&str>) -> bool {
    if DISSALLOWED_CONTENTS.contains_key(current) {
        if let Some(next) = next {
            if let Some(contents) = DISSALLOWED_CONTENTS.get(current) {
